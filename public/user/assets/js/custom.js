  ...
  $(document).ready(function () {
    ...
    $('.cart_btn').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                let cartHtml = '';
                Object.values(response.cart_items).forEach(item => {
                    cartHtml += `
                        <li>
                            ...
                            <div class="item_content">
                                ...
                                <span class="item_price">$${item.price} x ${item.quantity}</span>
                            </div>
                            <button type="button" class="remove_btn remove_item" data-id="${item.id}"><i class="fal fa-trash-alt"></i></button>
                        </li>
                    `;
                });

                $('.cart_items_list').html(cartHtml);
                $('.total_price_details').html(`
                    <li><span>Subtotal:</span><span>$${response.subtotal.toFixed(2)}</span></li>
                    <li><span>Vat 5%:</span><span>$${response.vat.toFixed(2)}</span></li>
                    <li><span>Discount 20%:</span><span>-$${response.discount.toFixed(2)}</span></li>
                    <li><span>Total:</span><span>$${response.total.toFixed(2)}</span></li>
                `);
            },
    ...
    $(document).on('click', '.remove_item', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    $('.cart_btn').click(); // Refresh cart after removal
