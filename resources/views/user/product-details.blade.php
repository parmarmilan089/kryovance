    ...
    <main>
       ...
       ================================================== -->
        <section class="breadcrumb_section minimal_breadcrumb d-flex align-items-end clearfix"
            data-background="{{ 'user/assets/images/breadcrumb/bg_11.jpg' }}">
                ...
                <div class="row mb_100 justify-content-lg-between">
                            ...
                            <div class="tab-content">
                                @foreach ($images as $key => $img)
                                    <div id="tab_{{ $key }}"
                                        ...
                                        <img src="{{ $img ? asset('storage/' . $img->file_path) : asset('user/assets/images/15980049.png') }}"
                                            alt="image_not_found">
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="col-lg-7 col-md-8">
                        <div class="shop_details_content">
                            ...
                            @php
                                // Get the discounted price using the helper function
                            ...
                            @if ($price !== $product->price)
                                ...
                                {{--  <span class="original_price">₹ {{ number_format($product->price, 2) }}</span>  --}}
                                <!-- Original Price -->
                            @else
              ...
              <li><a class="size_guide" href="#!"><i class="far fa-tape mr-1"></i> Size Guide</a></li>
                            ...
                            <form action="{{ route('add-to-cart', $product->id) }}" method="post" id="addToCartForm">
                                ...
                                <ul class="btns_group_1 ul_li mb_30 clearfix">
                                    ...
                                    @guest('customer')
                                        <li>
                                            <a class="custom_btn bg_black text-uppercase" href="{{ route('user-login') }}"><i
                                                    class="fal fa-user mr-2"></i>Login For Add to Cart</a>
                                        </li>
                                    @else
                                        <li>
                                            <button type="submit" class="custom_btn bg_black text-uppercase"
                                                href="{{ route('user-login') }}"><i class="fal fa-shopping-bag mr-2"></i> Add
                                                To Cart</button>
                                        </li>
                                    @endguest
                                </ul>
                ...
                $.ajax({
                    ...
                    error: function(response) {
                        $('#cart-message').removeClass('alert-success').addClass('alert-danger')
                            .text('Failed to add product to cart.').fadeIn().delay(3000)
                            .fadeOut();
                    }
                });
            });
