    ...
    <div class="card">
        <div class="card-body">
            ...
            <span style="float: right; margin-top: 10px;" >
                <a class="text-center btn btn-primary" href="{{URL::route('add-customer')}}" >Add Customer</a>
            </span>
            <table class="dt-multilingual table border-top" id="user_table" >
                <thead>
                    <tr>
...
$(document).ready(function () {
    var table = $('#user_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('json-userall') }}",
        columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            {data: "name", name: 'name'}, // Display last name
            { data: 'role_name', name: 'role_name' }, // 👈 Add this
            { data: 'username', name: 'username' }, // 👈 Add this
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    ...
                    url: admin_path + "/delete-customer",
                    type: "POST",
                    data: {delete_id: id},
