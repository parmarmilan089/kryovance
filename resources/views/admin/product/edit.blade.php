...
<div class="col-lg-12">
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form id="form_add_product" method="POST" action="{{ route('update-product') }}"
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <textarea name="feature_description" id="feature_description"
                            class="form-control">{!! $product->feature_description !!}</textarea>
                    </div>
                </div>
                ...
                <!-- For Seller -->
                <div class="row mb-3">
                    <label for="price_seller" class="col-sm-2 col-form-label">Seller Percentage</label>
                    <div class="col-sm-4">
                        <input type="number" class="form-control" id="price_seller" name="percentage_seller"
                            placeholder="Enter Seller Percentage"
                            value="{{ old('percentage_seller', $productDiscounts[1] ?? '') }}" />
                        @if ($errors->has('percentage_seller'))
                        <div class="error">{{ $errors->first('percentage_seller') }}</div>
                        @endif
                    </div>
                </div>
                ...
                <!-- For Distributor -->
                <div class="row mb-3">
                    <label for="percentage_distributor" class="col-sm-2 col-form-label">Distributor Percentage</label>
                    <div class="col-sm-4">
                        <input type="number" class="form-control" id="percentage_distributor"
                            name="percentage_distributor" placeholder="Enter Distributor Percentage"
                            value="{{ old('percentage_distributor', $productDiscounts[2] ?? '') }}" />
                        @if ($errors->has('percentage_distributor'))
                        <div class="error">{{ $errors->first('percentage_distributor') }}</div>
                        @endif
                    </div>
                </div>
                ...
                <!-- For Government -->
                <div class="row mb-3">
                    <label for="percentage_government" class="col-sm-2 col-form-label">Government Employee
                        Percentage</label>
                    <div class="col-sm-4">
                        <input type="number" class="form-control" id="percentage_government"
                            name="percentage_government" placeholder="Enter Government Employee Percentage"
                            value="{{ old('percentage_government', $productDiscounts[3] ?? '') }}" />
                        @if ($errors->has('percentage_government'))
                        <div class="error">{{ $errors->first('percentage_government') }}</div>
                        @endif
                    </div>
                </div>
                ...
                <!-- For Retailer -->
                <div class="row mb-3">
                    <label for="percentage_retailer" class="col-sm-2 col-form-label">Retailer Percentage</label>
                    <div class="col-sm-4">
                        <input type="number" class="form-control" id="percentage_retailer" name="percentage_retailer"
                            placeholder="Enter Retailer Percentage"
                            value="{{ old('percentage_retailer', $productDiscounts[4] ?? '') }}" />
                        @if ($errors->has('percentage_retailer'))
                        <div class="error">{{ $errors->first('percentage_retailer') }}</div>
                        @endif
                    </div>
                </div>
                ...
                <!-- For Wholesaler -->
                <div class="row mb-3">
                    <label for="percentage_wholesaler" class="col-sm-2 col-form-label">Wholesaler Percentage</label>
                    <div class="col-sm-4">
                        <input type="number" class="form-control" id="percentage_wholesaler"
                            name="percentage_wholesaler" placeholder="Enter Wholesaler Percentage"
                            value="{{ old('percentage_wholesaler', $productDiscounts[5] ?? '') }}" />
                        @if ($errors->has('percentage_wholesaler'))
                        <div class="error">{{ $errors->first('percentage_wholesaler') }}</div>
                        @endif
                    </div>
                </div>
                <div class="row mb-3">
                        ...
                        <div id="image-list" class="mt-3 d-flex flex-wrap">
                            ...
                            <div class="image-preview me-2 mb-2"
                                ...
                                <button type="button" class="btn btn-danger btn-sm remove-existing-image"
                                    style="position: absolute; top: 0px; right: 0px;" data-image-id="{{ $image->id }}">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#image').on('change', function () {
            const existingCount = $('#image-list .image-preview').length - filesArray
                .length; // Only count existing DB images
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $.each(data, function (key, value) {
                            $('#inventory_id').append('<option value="' + value.id +
                                '">' + value.model_name + ' - Qty: ' + value
                                    .qty + '</option>');
                        });
                    }
                });
